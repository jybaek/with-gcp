앞서 [CloudML 개념](./concept.md)에 대해서 살펴봤고, 이제 본격적으로 사용하는 방법에 대해서 기술하고자 한다. 우선 모든 예제는 우분투 16.04 에 *Cloud SDK* 를 설치한 환경에서 진행 되었다. *CloudML* 의 모든 것이 *gcloud* 명령어를 통해 진행되기 때문에 OS 의 환경에서 오는 차이는 없을 것이다.
